...
import { RoomMe, AccountMe, RentalRooms, RentalManagement, BillManagement } from '../../components/Section/MyAccount';
    ...
    return (
        <section className='border-0 border-t border-solid border-zinc-400 flex ' style={{ minHeight: 'calc(100svh - 2.35rem)' }}>
...

                    {
                        authData?.user?.role === "landlord" && (
                            <h3
                                ...
                                className={`border-0 border-b border-solid font-normal hover:underline cursor-pointer py-3 px-6 border-zinc-400 ${positionNum === 3 ? "underline" : ""
                                    }`}
                            >
                                Phòng cho thuê
                            </h3>
                    ...
                    }
                    {
                        authData?.user?.role === "landlord" && (
                            <h3
                                ...
                                className={`border-0 border-b border-solid font-normal hover:underline cursor-pointer py-3 px-6 border-zinc-400 ${positionNum === 4 ? "underline" : ""
                                    }`}
                            >
                                Quản lý cho thuê
                            </h3>
                    ...
                    }
                    {
                        authData?.user?.role === "landlord" && (
                            <h3
                                ...
                                className={`border-0 border-b border-solid font-normal hover:underline cursor-pointer py-3 px-6 border-zinc-400 ${positionNum === 5 ? "underline" : ""
                                    }`}
                            >
                                Hóa đơn
                            </h3>
            ...
            <div className='p-8 flex-1 min-h-[82vh]'>
                ...
                {positionNum === 3 && authData?.user?.role === "landlord" && <RentalRooms />}
                {positionNum === 4 && authData?.user?.role === "landlord" && <RentalManagement />}
                {positionNum === 5 && authData?.user?.role === "landlord" && <BillManagement />}
            </div>
        </section>
    );
}
