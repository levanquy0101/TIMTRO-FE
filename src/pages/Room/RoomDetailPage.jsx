    ...
    return (
        <>
            <Slider {...settings}>
                {(room?.images?.length
                    ? room.images
                    : Array(3).fill({ image_url: bgPlaceholder }) // Sử dụng ảnh placeholder nếu không có ảnh
                ).map((item, index) => (
            ...
            <section className='px-12'>
                <div className='relative flex gap-4'>
                    <div>
                        ...
                        <p>{room?.address}</p>
                        <p>{room?.bedrooms} giường - {room?.toilets} nhà vệ sinh</p>
                    ...
                    <div className='-mt-20 border-solid border-zinc-400 border-2 p-3 bg-white rounded min-w-96 max-w-min ml-auto'>
                        <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4 mt-8'>
                            <label>
                                ...
                                <input className='w-full p-2 outline-none' {...register("tenant_id")} disabled defaultValue={user?.name} />
                            ...
                            <label className='flex gap-1 whitespace-nowrap flex-col' >
                                ...
                                <input type="date" className='w-full p-2 outline-none' {...register("start_date", { required: "Trường là bắt buộc!" })} min={new Date().toISOString().split('T')[0]} />
                                <small className='text-red-500'>{errors.start_date?.message}</small>
                            </label>
                            <strong>Hợp đồng</strong>
                            <div className='flex gap-4'>
                                <label className='flex gap-1 whitespace-nowrap' >
                                    <input type="radio" value={1} {...register("end_date", { required: "Trường là bắt buộc!" })} />
                                    <b >1 Tháng</b>
                                </label>
                                <label className='flex gap-2 whitespace-nowrap' >
                                    <input type="radio" value={6} {...register("end_date", { required: "Trường là bắt buộc!" })} />
                                    <b >6 Tháng</b>
                                </label>
                                <label className='flex gap-2 whitespace-nowrap' >
                                    <input type="radio" value={12} {...register("end_date", { required: "Trường là bắt buộc!" })} />
                                    <b >1 Năm</b>
                                </label>
                            </div>
