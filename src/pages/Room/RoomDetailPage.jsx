        ...
        <>
            <Slider {...settings}>
                {(
                    room?.images?.length
                        ? room.images
                        : Array(3).fill({ image_url: bgPlaceholder }) // Tạo mảng 3 ảnh mặc định nếu không có ảnh
                ).map((item, index) => {
                    ...
                    return (
                        <div key={index} className="w-max">
                            <img
                                src={(isLoaded ? item?.image_url : bgPlaceholder) || bgPlaceholder} // Hiển thị ảnh hoặc placeholder
                                alt={`Slide ${index + 1}`}
